...
def client_id() -> str:
    try:
        ...
        return db.cred_value(
            "facebook", "client_id", os.environ.get("FACEBOOK_CLIENT_ID") or ""
        )
    except Exception:
...
def client_secret() -> str:
    try:
        ...
        return db.cred_value(
            "facebook",
            "client_secret",
            os.environ.get("FACEBOOK_CLIENT_SECRET") or "",
        )
    except Exception:
        return (
            (os.environ.get("FACEBOOK_CLIENT_SECRET") or "").strip()
...
def redirect_uri(request=None) -> str:
    ...
    return resolve_oauth_redirect(
        "facebook", request, os.environ.get("FACEBOOK_REDIRECT_URI") or ""
    )
