...
def client_id() -> str:
    try:
        ...
        return db.cred_value(
            "instagram", "client_id", os.environ.get("INSTAGRAM_CLIENT_ID") or ""
        )
    except Exception:
...
def client_secret() -> str:
    try:
        ...
        return db.cred_value(
            "instagram",
            "client_secret",
            os.environ.get("INSTAGRAM_CLIENT_SECRET") or "",
        )
    except Exception:
        return (
            (os.environ.get("INSTAGRAM_CLIENT_SECRET") or "").strip()
...
def redirect_uri(request=None) -> str:
    ...
    return resolve_oauth_redirect(
        "instagram", request, os.environ.get("INSTAGRAM_REDIRECT_URI") or ""
    )
